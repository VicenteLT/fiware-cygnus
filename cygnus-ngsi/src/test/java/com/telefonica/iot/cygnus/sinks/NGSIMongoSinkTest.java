...
public class NGSIMongoSinkTest {
    ...
    public void testNativeTypeColumnFalse() throws CygnusBadConfiguration {
        ...
        ngsiMongoSink.configure(createContextforNativeTypes(null, null, null, null, null, null, null, null, null, null, null, null, null, attr_native_types));
        // Create a NGSIEvent
        ...
        NGSIMongoSink.ColumnAggregator columnAggregator = ngsiMongoSink.new ColumnAggregator(false, false, false, false);
        ...
        System.out.println(columnAggregator.getDocArray().toString());
    ...
    public void testNativeTypeColumnBatch() throws CygnusBadConfiguration, CygnusRuntimeError, CygnusPersistenceError, CygnusBadContextData {
        ...
        try {
            ...
            while (batch.hasNext()) {
                ...
                NGSIGenericAggregator aggregator = ngsiMongoSink.getAggregator(false);
                aggregator.initialize(events.get(0));
                for (NGSIEvent event : events) {
                    aggregator.aggregate(event);
                } // for
                ArrayList<Document> documentos = ((NGSIMongoSink.ColumnAggregator) aggregator).getDocArray();
    ...
    public void testNativeTypeRow() throws CygnusBadConfiguration {
        ...
        headers.put(NGSIConstants.FLUME_HEADER_MAPPED_SERVICE, mappedService);
        headers.put(NGSIConstants.FLUME_HEADER_MAPPED_SERVICE_PATH, mappedServicePath);
        NGSIMongoSink.RowAggregator rowAggregator = ngsiMongoSink.new RowAggregator(false, false,false,true);
        NotifyContextRequest.ContextElement contextElement = createContextElementForNativeTypes();
        NGSIEvent ngsiEvent = new NGSIEvent(headers, contextElement.toString().getBytes(), contextElement, null);
        rowAggregator.initialize(ngsiEvent);
        rowAggregator.aggregate(ngsiEvent);
        System.out.println(rowAggregator.getDocArray());
